xmlport 78604 "BAC Export Trans Target 2018"
{
  ...
  {
  trigger OnBeforePassVariable()begin
    version:='1.2';
  end;
  }
  textelement(infile)
  ...
  tableelement(Target;
  "BAC Translation Target")
  {
  XmlName = 'trans-unit';

  fieldattribute(id;
  Target."Trans-Unit Id")
  {
  }
  fieldattribute("maxWidth";
  Target."Max Width")
  {
  }
  ...
  var TransNotes: Record "BAC Translation Notes";
  ...
  trigger OnPreXmlPort()var TempFile: Text;
  begin
    TransProject.Get(target.getfilter("Project Code"));
    TempFile:=TransProject."File Name";
    if StrPos(lowercase(TempFile), '.xlf') > 0 then currXMLport.Filename:=CopyStr(TempFile, 1, StrPos(lowercase(TempFile), '.xlf')) + Target.GetFilter("Target Language ISO code") + '.xlf';
    if StrPos(lowercase(TempFile), '.xlif') > 0 then currXMLport.Filename:=CopyStr(TempFile, 1, StrPos(lowercase(TempFile), '.xlif')) + Target.GetFilter("Target Language ISO code") + '.xlif';
  end;
  procedure SetProjectCode(inProjectCode: Code[10];
  InSourceLang: Text[10];
  InTargetLang: Text[10])begin
  ...
  local procedure CreateTranNote()begin
